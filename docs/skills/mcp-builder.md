...
当构建 MCP 服务器以集成外部 API 或服务时使用此技能，无论是在 Python (FastMCP) 还是 Node/TypeScript (MCP SDK)。用于创建高质量的 MCP（模型上下文协议）服务器，使 LLMs 能够通过设计良好的工具与外部服务交互。

---

## 概述

创建 MCP（模型上下文协议）服务器，使 LLMs 能够通过设计良好的工具与外部服务交互。MCP 服务器的质量取决于它使 LLMs 完成现实世界任务的能力。
...
**API 覆盖 vs. 工作流工具：**
在综合 API 端点覆盖和专业化工作流工具之间取得平衡。工作流工具对特定任务可能更方便，而综合覆盖给智能体组合操作的灵活性。性能因客户端而异——一些客户端受益于结合基本工具的代码执行，而其他客户端使用更高级别工作流更好。不确定时，优先综合 API 覆盖。

**工具命名和可发现性：**
清晰、描述性的工具名称帮助智能体快速找到正确的工具。使用一致的前缀（例如 `github_create_issue`、`github_list_repos`）和动作导向命名。

**上下文管理：**
智能体受益于简洁的工具描述以及过滤/分页结果的能力。设计返回专注、相关数据的工具。一些客户端支持代码执行，这可以帮助智能体高效过滤和处理数据。

**可操作错误消息：**
错误消息应该用具体建议和后续步骤指导智能体走向解决方案。
...
然后获取带有 `.md` 后缀的特定页面（例如 `https://modelcontextprotocol.io/specification/draft.md`）。
...
**推荐技术栈：**
- **语言**：TypeScript（高质量 SDK 支持，在许多执行环境例如 MCPB 中兼容性好。再加上 AI 模型擅长生成 TypeScript 代码，受益于其广泛使用、静态类型和良好的 linting 工具）
- **传输**：远程服务器使用可流式 HTTP，使用无状态 JSON（更简单可扩展和维护，而不是有状态会话和流式响应）。本地服务器使用 stdio。
...
**工具选择：**
优先综合 API 覆盖。列出要实现的端点，从最常见的操作开始。
...
- 使用现代 SDK 的工具响应中的 `structuredContent`
- 帮助客户端理解和处理工具输出
...
**加载 [✅ 评估指南](./reference/evaluation.md) 了解完整评估指南。**

#### 4.1 理解评估目的

使用评估来测试 LLMs 是否可以有效使用您的 MCP 服务器回答现实、复杂的问题。
...
### 核心 MCP 文档（首先加载）
- **MCP 协议**：从 `https://modelcontextprotocol.io/sitemap.xml` 开始，然后获取带有 `.md` 后缀的特定页面
